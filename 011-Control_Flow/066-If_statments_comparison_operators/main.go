...
func main() {
	...
	y := 5 //this is the fourth statement to Println
	fmt.Printf(" x=%v \n y=%v\n", x, y)
	...
	*/
	...
	if z := 2 * rand.Intn(x); z>= x {
		fmt.Printf("z is %v and that is GREATER THAN OR EQUAL x which is %v/n", z, x)
	} else {
		fmt.Printf("z is %v and that is LESS THAN x which is %v/n", z, x)
	}
	...
	case 40:
		...
		fallthrough
	case 41:
		fmt.Println("printed because of ALL OF THE fallthrough: x is 41")
		fallthrough
	case 42:
		fmt.Println("printed because of ALL OF THE fallthrough: x is 42")
		fallthrough
	case 43:
		fmt.Println("printed because of ALL OF THE fallthrough: x is 43")
		fallthrough
	default:
		fmt.Println("printed because of ALL OF THE fallthrough: this is the default case for x")
	}
	...
	Thera are three forms, only one of which has semicolons.
	...
	*/
	...
	for i := 0; i < 5; i++ {
		fmt.Println("--")
		for j := 0; j < 5; j++ {
			fmt.Printf("outer loop %v \t inner loop %v", i, j)
		}
	}
